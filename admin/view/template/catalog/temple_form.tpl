      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-temple" class="form-horizontal">
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-sect"><?php echo $entry_sect; ?></label>
            <div class="col-sm-10">
              <select name="sect_id" id="input-sect" class="form-control">
                <?php foreach ($sects as $sect) { ?>
                <?php if ($sect['id'] == $sect_id) { ?>
                <option value="<?php echo $sect['id']; ?>" selected="selected"><?php echo $sect['name']; ?></option>
                <?php } else { ?>
                <option value="<?php echo $sect['id']; ?>"><?php echo $sect['name']; ?></option>
                <?php } ?>
                <?php } ?>
              </select>
            </div>

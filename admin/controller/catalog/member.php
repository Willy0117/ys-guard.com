...
class ControllerCatalogMember extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/member', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['insert'] = $this->url->link('catalog/member/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/member/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$member_total = $this->model_catalog_member->getTotalMembers();

		$results = $this->model_catalog_member->getMembers($filter_data);
		...
		foreach ($results as $result) {
			$data['members'][] = array(
				...
				'edit'            => $this->url->link('catalog/member/edit', 'token=' . $this->session->data['token'] . '&member_id=' . $result['member_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/member', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/member', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/member', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/member_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['member_id'])) {
			$data['action'] = $this->url->link('catalog/member/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/member/edit', 'token=' . $this->session->data['token'] . '&member_id=' . $this->request->get['member_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/member', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/member_form.tpl', $data));
	}

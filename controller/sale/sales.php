...
class ControllerSaleSales extends Controller {
	...
	*/
	public function excel() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_slip'])) {
				$url .= '&filter_slip=' . urlencode(html_entity_decode($this->request->get['filter_slip'], ENT_QUOTES, 'UTF-8'));
			}		
    ...
    */
	protected function getList() {
		...
		if (isset($this->request->get['filter_slip'])) {
			$filter_slip = $this->request->get['filter_slip'];
		} else {
			$filter_slip = null;
		}
		...
		if (isset($this->request->get['filter_purpose'])) {
			$filter_purpose = $this->request->get['filter_purpose'];
		} else {
			$filter_purpose = '';
		}
		...
		if (isset($this->request->get['filter_purpose'])) {
			$url .= '&filter_purpose=' . $this->request->get['filter_purpose'];
		}
		...
		foreach ($results as $result) {
			...
			$purpose = '';
			foreach($data['purposes'] as $value) {
				if ($value['id'] == $result['purpose'] ) $purpose = str_replace('料金', '', $value['name']);
			}
			...
			$data['sales'][] = array(
				...
				'id'           => $result['id'],
				'purpose'      => $purpose,
				'total'        => number_format($result['total1'] + $result['tax1'] + $result['total2'] + $result['tax2']),
		...
		$data['column_purpose'] = $this->language->get('column_purpose');
		$data['column_total'] = $this->language->get('column_total');
		
		$data['column_action'] = $this->language->get('column_action');

		$data['entry_name'] = $this->language->get('entry_name');			// 喪家名
		$data['entry_slip'] = $this->language->get('entry_slip');			// 伝票No.
		...
		$data['sort_purpose'] = $this->url->link('sale/sales', 'token=' . $token . '&sort=purpose' . $url, 'SSL');
